...
// Busy time ranges on the calendar for one date (business timezone), as
// [{ start, end }] epoch ms. This is what keeps the site synced with the
// calendar: ANY event there - booked online or added by hand - blocks slots.
async function busyRangesForDate(dateStr, schedule) {
  const id = calendarId();
  const timeMin = new Date(slots.epochFor(dateStr, '00:00', schedule.timezone)).toISOString();
  const timeMax = new Date(slots.epochFor(dateStr, '23:59', schedule.timezone) + 60 * 1000).toISOString();
  const data = await api('POST', API + '/freeBusy', { timeMin, timeMax, items: [{ id }] });
  const calendars = data.calendars || {};
  const entry = calendars[id] || calendars[Object.keys(calendars)[0]] || {};
  return (entry.busy || []).map((b) => ({ start: Date.parse(b.start), end: Date.parse(b.end) }));
}

module.exports = { isConfigured, status, testConnection, createBookingEvent, deleteBookingEvent, busyRangesForDate };
